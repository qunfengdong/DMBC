...
best_cm <- function(CV=cv, auc_out=auc_out){
  bestrank <- auc_out$NumberOfFeature[which.max(auc_out$AUC_Prior_Area)]
  tmp <-lapply(cv,function(x) x[x$feature_rank==bestrank,])
  file_rank = data.frame(Reduce(function(...) merge(...,all=T),tmp))

  pred <- as.numeric(as.numeric(as.vector(file_rank$Poster_Prio_Type1)) > as.numeric(as.vector(file_rank$Poster_Prio_Type2)))
