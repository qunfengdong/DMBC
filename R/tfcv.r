...
tfcv <- function(data=data, type_col=2, col_start=3,Cutoff_mean=0.0001,Cutoff_ratio=0.1,totalReadsCutoff=500, Cutoff_pvalue = 1){
  res <- list()
  ...
  for (i in 1:length(flds)){
    print (paste0("Current working section: ", i))
    testSet <- data[-flds[[i]],]
